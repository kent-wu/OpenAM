...
import org.forgerock.openam.selfservice.stage.IssueCodeConfig;
import org.forgerock.openam.selfservice.stage.WeiboUserDetailsConfig;
 ...
 * @since 13.0.0
 */
public final class WeiboUserRegistrationConfigProvider
        implements ServiceConfigProvider<WeiboUserRegistrationConsoleConfig> {

    @Override
    public boolean isServiceEnabled(WeiboUserRegistrationConsoleConfig config) {
        return config.isEnabled();
    }

    @Override
    public ProcessInstanceConfig getServiceConfig(
            WeiboUserRegistrationConsoleConfig config, Context context, String realm) {

        List<StageConfig> stages = new ArrayList<>();
        
        stages.add(new WeiboUserDetailsConfig());

        stages.add(new UserRegistrationConfig()
                .setIdentityServiceUrl("/users"));
        
        stages.add(new IssueCodeConfig());
